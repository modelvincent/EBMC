...
float consigne_steering=0;
float previous_steering=0;
AsyncWebServer server(80);

int previous_Channel1=0;
int previous_Channel2=0;

int previous_Channel5=0;
int previous_Channel6=0;
...
MD_RobotEyes::emotion_t current_emotion=MD_RobotEyes::E_NEUTRAL;
MD_RobotEyes::emotion_t previous_emotion=MD_RobotEyes::E_NEUTRAL;
...
float CalculADC(int Value)
{
  float Result = (float)Value *((3.3/4096)*((R1+R2))/R2);
  return Result;
}
...
void setup() {
  ...
  // Send a GET request to <ESP_IP>/?fader=<inputValue>
    ...
    // Get value for Buzzer
      ...
      {
         current_emotion=MD_RobotEyes::E_WINK;
      }
      else
      {
          current_emotion=MD_RobotEyes::E_NEUTRAL;
      }
    }
    // Get value for Led
      ...
      inputMessage = PARAM_PUSH4;
       if(inputValue.equals("1"))
        {
         current_emotion=MD_RobotEyes::E_EVIL;
        }
        else
        {
          current_emotion=MD_RobotEyes::E_NEUTRAL;
  ...
  E.setAutoBlink(true);
  E.setBlinkTime(1000);
  E.setAnimation(MD_RobotEyes::E_NEUTRAL, true);
...
void loop() {
  ...
  {
    Serial.println("changement d'emotion");
    E.setAnimation(current_emotion, true);  
  ...
  for (byte channel = 1; channel <= channelAmount; ++channel) 
  {
    ...
    if(value >1000 && value < 2000)
    {
      if(channel==1)
      {
        if(previous_Channel1 != value)
        {
          /*Serial.print("Changement de channel 1 car previous_channel=");
          Serial.print(previous_Channel0);
          Serial.print(" et value = ");
          Serial.println(value);*/
          OSCfader[1]=(((float)value-1000.0)/1000.0);
          OSCnewMessage=1;
          OSCpage=1;
          previous_Channel1=value;
        }
      }
      if(channel==2)
      {
        if(previous_Channel2 != value)
        {
      /*    Serial.print("Changement de channel 2 car previous_channel=");
          ...
          OSCfader[0]=(((float)value-1000.0)/1000.0);
          OSCnewMessage=1;
          OSCpage=1;
          previous_Channel2=value;
        }
      }
  ...
  if (MPU6050_newData()) {
    ...
    {
      // Ajout pour limiter les acceleration lors des changements de direction
      float EcartConsigne=5;
      if(consigne_steering>0)
      {
        if(steering < consigne_steering)
        {
          //steering=(steering+(consigne_steering*0.1));
          steering=steering+EcartConsigne;
        }
        else
        {
          steering=consigne_steering;
        }
      }
      ...
      if(consigne_steering==0)
      {
        if(steering>=EcartConsigne)
        {
          steering=steering-EcartConsigne;  
        }
        else if( steering <= -EcartConsigne)
        {
          steering=steering+EcartConsigne;
        }
        else
...
void processOSCMsg() {
  ...
  if (OSCpage == 1) {
    ...
    if (OSCmove_mode) {
      ...
      consigne_steering = OSCfader[1] - 0.5;
      if (consigne_steering > 0)
        consigne_steering = (consigne_steering * consigne_steering + 0.5 * consigne_steering) * max_steering;
      else
        consigne_steering = (-consigne_steering * consigne_steering + 0.5 * consigne_steering) * max_steering;
    }
